...
5. [Адаптивный дизайн с Bootstrap](#адаптивный-дизайн-с-bootstrap)
5. [Кеширование данных](#кеширование-данных)
6. [Использование API Wikipedia](#использование-api-wikipedia)
7. [Использование карты и библиотеки Leaflet](#использование-карты-и-библиотеки-leaflet)
8. [Доступ к проекту](#доступ-к-проекту)
...
WeatherSphereApp использует фреймворк **Bootstrap** для обеспечения адаптивности и корректного отображения на различных устройствах, включая мобильные телефоны, планшеты и компьютеры. Это означает, что приложение автоматически подстраивается под размер экрана пользователя, обеспечивая удобную работу как на больших, так и на маленьких экранах.

###  Преимущества адаптивного дизайна:

1. **Поддержка всех устройств**: Интерфейс корректно отображается на смартфонах, планшетах и компьютерах, без необходимости создания отдельных версий сайта.
2. **Мобильная веб-версия**: Благодаря встроенным медиазапросам в Bootstrap, страницы и элементы приложения автоматически подстраиваются под разрешение экрана пользователя, обеспечивая удобную навигацию и доступ к функционалу даже на маленьких экранах.
3. **Гибкость и универсальность**: Множество готовых компонентов Bootstrap (таких как сетки, кнопки, формы и выпадающие меню) делают создание адаптивного дизайна быстрым и удобным.

- Элементы навигационного меню и формы поиска автоматически перестраиваются для удобного использования на мобильных устройствах, превращаясь в выпадающее меню на малых экранах.
- Карта и погодные данные также подстраиваются под размеры экрана, что позволяет пользователям получать актуальную информацию на любых устройствах.
...
#### Как это работает( Код из `weather.js 88`)
...
В коде `views.py 18` API для погоды используется следующий URL для запроса информации о городе:
```python
...
### Использование карты и библиотеки Leaflet( Код из `weather.js 70`)
...
1. **Создайте новый проект** в PyCharm. При создании проекта виртуальное окружение будет создано автоматически.



2. **Клонируйте репозиторий** в свой локальный каталог, используя команду:

```bash
   git clone https://github.com/MRGiorgiosDev8/weather_django.git
 ```
...
5. Запустите сервер **Django** командой:
